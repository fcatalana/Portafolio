...
import com.FeriaVirtual.MaipoGrande.Entidad.ProductoParaVender;
import com.FeriaVirtual.MaipoGrande.Entidad.ProductoVendido;
import com.FeriaVirtual.MaipoGrande.Entidad.Venta;
import com.FeriaVirtual.MaipoGrande.Repository.ProductosRepository;
import com.FeriaVirtual.MaipoGrande.Repository.ProductosVendidosRepository;
...
public class VenderController {
    ...
    public String terminarVenta(HttpServletRequest request, RedirectAttributes redirectAttrs) {
        ...
        for (ProductoParaVender productoParaVender : carrito) {
            ...
            ProductoVendido productoVendido = new ProductoVendido(productoParaVender.getCantidad(), productoParaVender.getValor(), productoParaVender.getNombre_producto(), productoParaVender.getCodigo(), v);
            // Y lo guardamos
    ...
    public String agregarAlCarrito(@ModelAttribute Producto producto, HttpServletRequest request, RedirectAttributes redirectAttrs) {
        ...
        if (!encontrado) {
            carrito.add(new ProductoParaVender(productoBuscadoPorCodigo.getNombre_producto(), productoBuscadoPorCodigo.getValor(), productoBuscadoPorCodigo.getExistencia(), productoBuscadoPorCodigo.getCodigo(),productoBuscadoPorCodigo.getId_producto(), 1f));
        }
        this.guardarCarrito(carrito, request);
        return "redirect:/vender/";
    }
